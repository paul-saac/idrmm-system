...
// Where the back chevron returns to — the Equipment tab's Requests
// sub-tab specifically, not just the project root.
function equipmentRequestsListHref(projectId: number) {
  return `/admin/projects/${projectId}?tab=equipment&subtab=requests`;
}
...
function PriorityBadge({ priority }: { priority: EquipmentRequestPriority }) {
  const styles: Record<EquipmentRequestPriority, string> = {
    routine: "bg-zinc-100 text-zinc-600",
    urgent: "bg-amber-50 text-amber-700",
    emergency: "bg-red-50 text-red-700",
  };
  const labels: Record<EquipmentRequestPriority, string> = {
    routine: "Routine",
    urgent: "Urgent",
    emergency: "Emergency",
  };
...
function FulfilmentBadge({ fulfilment }: { fulfilment: EquipmentRequestItemFulfilment }) {
  const styles: Record<EquipmentRequestItemFulfilment, string> = {
    not_fulfilled: "bg-zinc-100 text-zinc-500",
    partially_fulfilled: "bg-amber-50 text-amber-700",
    fulfilled: "bg-emerald-50 text-emerald-700",
  };
  const labels: Record<EquipmentRequestItemFulfilment, string> = {
...
const actionInitialState: EquipmentRequestActionState = {};

function ApproveButton({ requestId, projectId }: { requestId: number; projectId: number }) {
  const boundAction = approveEquipmentRequest.bind(null, requestId, projectId);
...
function CancelButton({ requestId, projectId }: { requestId: number; projectId: number }) {
  const boundAction = cancelEquipmentRequest.bind(null, requestId, projectId);
  const [state, formAction, pending] = useActionState(boundAction, actionInitialState);
  return (
    <span className="relative inline-flex">
      <form
...
export function EquipmentRequestDetailView({
  request,
}: {
    ...
    <>
      <header className="flex items-center justify-between border-b border-zinc-200 bg-white px-8 py-5">
        <nav className="flex items-center gap-1.5 text-sm">
          ...
          <Link
            href={equipmentRequestsListHref(projectId)}
            className="text-zinc-500 transition hover:text-zinc-900"
          >
            Equipment
          </Link>
          ...
          <span className="text-zinc-300">/</span>
          <span className="font-medium text-zinc-900">{request.erNo}</span>
        </nav>
        <LogoutButton />
      </header>
